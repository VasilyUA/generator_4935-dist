...
def generate_accepted_position_report(data_for_soldier, data_for_commander, data_for_higher_commander):
    doc = Document()
    ...
    add_paragraph_with_style(doc, "")
    add_paragraph_with_style(doc, "")
    add_paragraph_with_style(doc, f"Дійсним доповідаю, що справи та посаду {data_for_soldier.get('position_full_genitive', '').lower()} {FULL_MILITARY_UNIT} прийняв.", first_line_indent=34)
    ...
    doc_name = os.path.join(OUTPUT_DIR, f"{data_for_soldier.get('name_nominative')} прийняв посаду.docx")
    doc.save(doc_name)
